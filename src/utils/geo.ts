...
export function findClosestPointOnTrail(
  ...
  if (typeof movementBearingOrOptions === 'object' && movementBearingOrOptions !== null) {
    ...
    movementBearing = movementBearingOrOptions;
...
export function getRelativeDirectionText(
  ...
  const roundedDist = Math.max(1, Math.round(distanceToTarget));

  if (isReturningToTrail) {
    return {
      headline: `Return to Trail (${roundedDist} m)`,
  ...
  return {
    headline: turnText,
    subline: `${roundedDist} m ahead along trail`,
  };
}
